...
struct OOTDCalendarView: View {
    ...
    @State private var showingNeglectedItems = false // 잠자고 있는 옷 시트 제어
                    ...
                    ForEach(generateDays(), id: \.self) { date in
                        if let date = date {
                            DayCell(date: date, 
                                ...
                                .onTapGesture {
                                    viewModel.send(intent: .selectDate(date))
                                }
                        } else {
                            // 날짜가 없는 빈 칸 (월 시작 전 여백)
                            Spacer().frame(height: 80)
                        }
                    }
                }
                .padding(.horizontal)
                .animation(.easeInOut, value: viewModel.state.currentMonth)
        ...
        VStack(alignment: .leading, spacing: 20) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    Text(viewModel.state.selectedDate, format: .dateTime.day().month(.wide))
                        .font(.system(size: 18, weight: .bold))
                    Text(viewModel.state.selectedDate, format: .dateTime.weekday(.wide))
                        .font(.system(size: 14))
                        .foregroundColor(.secondary)
                }
                Spacer()
        ...
        return VStack(alignment: .leading, spacing: 25) {
            ...
            if !insights.neglected.isEmpty {
                Button(action: { showingNeglectedItems = true }) {
                    HStack {
                        VStack(alignment: .leading, spacing: 5) {
                            Text("관심이 필요한 옷")
                                .font(.system(size: 15, weight: .bold))
                                .foregroundColor(.black)
                            Text("\(insights.neglected.count)개의 옷을 이번 달에 입지 않았어요.")
                                .font(.system(size: 13))
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                    ...
                    .padding()
                    .background(Color.fashionGray.opacity(0.8))
                    .cornerRadius(15)
                }
            }
        }
    ...
    private func generateDays() -> [Date?] {
        guard let monthRange = calendar.range(of: .day, in: .month, for: viewModel.state.currentMonth),
              let firstDayOfMonth = calendar.date(from: calendar.dateComponents([.year, .month], from: viewModel.state.currentMonth)) else {
            return []
        }
        
        let firstWeekday = calendar.component(.weekday, from: firstDayOfMonth)
        let offset = firstWeekday - 1 // 1: 일요일, 2: 월요일 ...
        
        var days: [Date?] = Array(repeating: nil, count: offset) // 시작 요일 전까지 빈 칸 채움
        ...
        VStack(spacing: 6) {
            ...
            ZStack {
                if let firstSnapshot = log?.itemSnapshots.first {
                    Group {
                        if let data = firstSnapshot.imageData, let uiImage = UIImage(data: data) {
                            Image(uiImage: uiImage)
                                .resizable()
                                .aspectRatio(contentMode: .fill)
                        } else {
                    ...
                    RoundedRectangle(cornerRadius: 8)
                        .fill(Color.fashionGray.opacity(0.3))
                        .frame(width: 40, height: 55)
                }
            }
        }
...
struct AddClothingToOOTDView: View {
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                // 상단 설명
                Text("\(selectedDate, format: .dateTime.day().month().weekday()) 코디 기록")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                    .padding(.vertical, 10)
                    ...
                    ScrollView {
                        LazyVGrid(columns: columns, spacing: 15) {
                            ForEach(allItems) { item in
                                ClothingSelectionCard(item: item, isSelected: selectedIds.contains(item.id))
                                    .onTapGesture {
                ...
                // 하단 저장 버튼
                Button(action: { onSave(Array(selectedIds)) }) {
                    Text("\(selectedIds.count)개 옷 선택 완료")
                        .font(.system(size: 16, weight: .bold))
                        .foregroundColor(.white)
                        .frame(maxWidth: .infinity)
                        .padding(.vertical, 16)
                        .background(selectedIds.isEmpty ? Color.gray : Color.black)
                        .cornerRadius(15)
                }
                .disabled(selectedIds.isEmpty)
                .padding(20)
            }
            .navigationTitle("옷 선택하기")
            ...
            .onAppear {
                // 옷장 데이터 로드
                self.allItems = WardrobeRepository.shared.getAllItems()
            }
        }
    }
}
